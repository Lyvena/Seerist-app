...
export function RadarGraphic() {
  return (
    <div className="relative mx-auto w-full max-w-[560px]">
      <div className="absolute -inset-10 rounded-[48px] bg-violet-500/10 blur-3xl" aria-hidden="true" />
      <div className="relative overflow-hidden rounded-[32px] border border-white/60 bg-white/75 p-4 shadow-[var(--shadow-lg)] backdrop-blur-xl">
        <div className="absolute inset-0 bg-[linear-gradient(rgba(124,58,237,0.06)_1px,transparent_1px),linear-gradient(90deg,rgba(124,58,237,0.06)_1px,transparent_1px)] bg-[size:28px_28px] opacity-60" aria-hidden="true" />
...
{SIGNALS.map((signal, index) => {
               ...
               return (
                 <motion.div
                   ...
                   animate={{ y: [0, -8, 0] }}
                 >
                   <div className="relative flex items-center gap-2 rounded-full border border-white bg-white/90 px-3 py-2 shadow-lg backdrop-blur">
            ...
            <div className="relative z-10 flex h-36 w-36 items-center justify-center rounded-full border border-violet-100 bg-white/80 shadow-xl backdrop-blur md:h-44 md:w-44">
...
function Metric({ label, value }: { label: string; value: string }) {
  return (
    <div className="rounded-2xl border border-violet-100 bg-white/70 p-3 text-center backdrop-blur">
      <p className="text-xl font-semibold text-slate-900" style={{ fontFamily: "var(--font-heading)" }}>{value}</p>
